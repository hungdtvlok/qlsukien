...
app.delete("/api/unregisterEvent", async (req, res) => {
    try {
        const { username, eventId } = req.query; // Lấy từ query, không phải body
        if (!username || !eventId) return res.status(400).json({ message: "Thiếu username hoặc eventId" });

        const user = await User.findOne({ username });
        if (!user) return res.status(404).json({ message: "User không tồn tại" });

        const deleted = await Registration.findOneAndDelete({ userId: user._id, eventId });
        if (!deleted) return res.status(404).json({ message: "Không tìm thấy đăng ký để hủy" });
